  ...
  return (
    <div className="w-full flex flex-col items-center justify-center min-h-[70vh]">
      ...
      <div className="glass-panel p-10 rounded-[2.5rem] w-full max-w-xl relative z-10 overflow-hidden neon-glow-violet text-center flex flex-col items-center border-t-electric-violet/30">
        ...
        <h2 className="text-3xl font-bold mb-2 tracking-tight">{t("sync.title")}</h2>
        ...
        <div className="relative mb-10">
          <AnimatePresence mode="wait">
            {syncState === "waiting" || syncState === "scanning" ? (
              ...
              >
                <QRCode 
                  value="https://t.me/moliya365_bot?start=auth_12345" 
                  size={200}
                  bgColor="#ffffff"
                  fgColor="#050505"
                  level="Q"
              ...
              >
                ...
                <h3 className="text-xl font-bold">{t("sync.success")}</h3>
        ...
        <div className="flex gap-4">
          {syncState === "waiting" && (
            <button 
              ...
              className="bg-electric-violet text-white px-8 py-3 rounded-full font-semibold hover:bg-electric-violet/90 transition shadow-[0_0_15px_rgba(139,92,246,0.4)] flex items-center gap-2 cursor-pointer"
            >
              <ScanLine size={18} /> {t("sync.scanBtn")}
            </button>
          )}
          ...
          {syncState === "connected" && (
            <button 
              ...
              className="bg-white/10 hover:bg-white/20 border border-white/10 text-white px-8 py-3 rounded-full font-semibold transition flex items-center gap-2 cursor-pointer"
            >
              <RefreshCcw size={18} /> {t("sync.reconBtn")}
            </button>
          )}
        </div>
      ...
      <div className="mt-12 flex gap-12 text-white/40 z-10">
        <div className="flex flex-col items-center gap-2">
          <Smartphone size={24} />
          <span className="text-xs">{t("sync.step1")}</span>
        </div>
        <div className="h-full border-r border-white/10" />
        <div className="flex flex-col items-center gap-2">
          <ScanLine size={24} />
          <span className="text-xs">{t("sync.step2")}</span>
        </div>
        <div className="h-full border-r border-white/10" />
        <div className="flex flex-col items-center gap-2">
          <WifiHigh size={24} />
          <span className="text-xs">{t("sync.step3")}</span>
        </div>
      </div>
